...
import listgamesCommand from "./listgames";
...
interface CommandModule {
    data: CommandData;
    execute: (data: { interaction: InteractionType }) => Promise<void>;
}

export async function registerCommands(client: Client) {
    const commands: any[] = [];
    const commandMap: Collection<string, CommandModule> = new Collection();
    const commandModules = [listgamesCommand];
    ...
    for (const commandModuleUntyped of commandModules) {
        ...
        try {
            ...
            logger.error(`Failed to load command "${commandModule?.data?.name ?? "unknown"}"`);
